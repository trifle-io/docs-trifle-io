...
`Trifle.Stats.Nocturnal` is the bucketing engine used by `track`, `assert`, and `values`. Most applications do not call it directly, but its rules determine the timestamp stored for every metric bucket.

Writing and reading use the same process:

1. Shift the input instant into the configured `time_zone`.
2. Floor it to the requested granularity.
3. Resolve that local boundary with `time_zone_database`.
...
Elapsed units measure real time. Calendar units preserve the local wall-clock fields while allowing the UTC and standard offsets to change.
...
Offsets are reconstructed through the configured timezone database for every target boundary:
...
- If calendar movement targets an ambiguous local time, Nocturnal prefers the occurrence with the source timestamp's total UTC offset and otherwise uses the earlier occurrence.
...
Use an IANA timezone such as `Europe/Bratislava`, not a fixed offset. A fixed offset has no transition rules.

## Timezone support

Nocturnal accepts timestamps representing any instant and shifts them into the configured `time_zone` before calculating bucket boundaries. IANA timezones include historical and future offset rules, so the same configuration handles standard time, daylight saving time, and transitions that are not exactly one hour. Configure a timezone database that provides those rules.

> Treat `time_zone` as part of the persisted metric schema. Set it before writing data and do not change it for an existing dataset.

The timezone name is not stored with each bucket. Trifle stores the absolute instant produced after flooring in the configured timezone. Changing `time_zone` does not rewrite those persisted buckets: subsequent reads build their lookup timestamps using the new local boundaries, which can produce different Unix timestamps. The original data still exists, but it may no longer be returned and can appear as empty or split buckets. Some boundaries may happen to coincide, but that is not a safe compatibility guarantee.
...
The Elixir implementation exposes the primitives used by the public operations:

```elixir
parser = Trifle.Stats.Nocturnal.Parser.new("1d")
nocturnal = Trifle.Stats.Nocturnal.new(at, config)

bucket = Trifle.Stats.Nocturnal.floor(nocturnal, parser.offset, parser.unit)
tomorrow = Trifle.Stats.Nocturnal.add(nocturnal, 1, :day)
